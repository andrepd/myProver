...
let ident = take_while1 (function 'a'..'z' | 'A'..'Z' | '_' -> true | _ -> false) 

let parens p = char '(' *> p <* char ')'

let comma = char ','

let pred_op = choice [string "="]
...
let parser_term : string term t =
  fix (fun expr ->
    ident >>= (fun x -> 
    (
      parens (sep_by comma expr) >>| (fun terms -> Func (x, terms))
    ) <|> (
      return @@ Var x
    )
    )
...
let parser_atom : string atom t =
  eat_spaces *> (
    (
      ident >>= (fun x ->
      parens (sep_by comma parser_term) >>| (fun terms -> 
        Pred (x, terms)
      ))
    ) <|> (
      parser_term >>= (fun x ->
...
let op_and = eat_spaces *> (string "&") *> return (fun x y -> And (x,y))
let op_or  = eat_spaces *> (string "|") *> return (fun x y -> Or  (x,y))
let op_imp = eat_spaces *> (string "=>") *> return (fun x y -> Imp (x,y))
let op_iff = eat_spaces *> (string "==") *> return (fun x y -> Iff (x,y))
...
let parse_formula s =
  match parse_string (eat_spaces *> (parser_boolean())) s with
  | Ok res -> res
  | Error msg -> failwith msg
